...
const modalPagoInfo   = ref<{ id: number; pedidoId?: number; monto: number; metodo: string } | null>(null)
...
function cerrarModal() {
  ...
  setTimeout(() => { modalImg.value = ''; modalPagoInfo.value = null }, 300)
}
  ...
  <section class="admin-wrap">
    ...
    <div class="kpis-grid">
      ...
      <div class="kpi-card kpi-featured">
        <div class="kpi-icon" style="background:linear-gradient(135deg,#e91e8c,#f06292); color:white">
      ...
      <!-- En revisión -->
      <div class="kpi-card kpi-clickable" @click="filtroEstado = 'en_revision'; paginaActual = 1">
      ...
      <!-- Rechazados -->
      <div class="kpi-card kpi-clickable" @click="filtroEstado = 'rechazado'; paginaActual = 1">
    ...
    <div v-if="Object.keys(kpis.porMetodo).length" class="metodo-resumen">
      ...
      <div class="metodo-chips">
        <div
          v-for="(monto, metodo) in kpis.porMetodo" :key="metodo"
          class="metodo-chip-kpi"
          :style="`background:${metodoCfg(metodo as string).bg}; color:${metodoCfg(metodo as string).color}`"
        >
          <span>{{ metodoCfg(metodo as string).icon }}</span>
          <span class="mc-label">{{ metodoCfg(metodo as string).label }}</span>
    ...
    <div class="filtros-wrap">
      ...
      <div class="filtro-tabs">
        <button
          v-for="e in ESTADOS_FILTRO" :key="e.value"
          class="tab-btn" :class="{ activo: filtroEstado === e.value }"
          @click="filtroEstado = e.value; paginaActual = 1"
        >
          {{ e.label }}
          <span class="tab-count">
            {{ e.value === 'todos'
              ? pagos.length
              : pagos.filter(p => p.estado === e.value).length }}
          </span>
        </button>
      </div>
      ...
      <div class="metodo-filtros">
        <button
          v-for="m in METODOS_FILTRO" :key="m.value"
          class="metodo-chip-btn"
          :class="{ activo: filtroMetodo === m.value }"
          @click="filtroMetodo = m.value; paginaActual = 1"
        >
          {{ m.icon }} {{ m.label }}
        </button>
      </div>
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-for="p in paginados" :key="p.id">
                ...
                <!-- ID Pedido -->
                <td>
                  <span class="id-badge pedido">#{{ String(p.pedido?.id ?? '—').padStart(5,'0') }}</span>
                </td>
                ...
                <!-- Estado pedido -->
                <td>
                  <span v-if="p.pedido?.estado" class="estado-badge estado-pedido"
              ...
              <tr v-if="!paginados.length">
                <td colspan="9" class="td-vacio">
                  <div class="vacio-wrap">
                    ...
                    <p>No hay pagos {{ filtroEstado !== 'todos' ? `con estado "${filtroEstado}"` : '' }}</p>
                    <button
                      v-if="filtroEstado !== 'todos' || filtroMetodo !== 'todos'"
                      class="btn-reset"
                      @click="filtroEstado = 'todos'; filtroMetodo = 'todos'; q = ''"
                    >
                      Limpiar filtros
                    </button>
                  </div>
        ...
        <div v-if="showModalImg" class="modal-overlay" @click.self="cerrarModal">
          <div class="modal-img-wrap">
            ...
            <div v-if="modalPagoInfo" class="modal-info-bar">
              ...
              <div class="modal-info-item">
                <span>Método</span>
                <span class="metodo-badge small"
                  :style="`background:${metodoCfg(modalPagoInfo.metodo).bg}; color:${metodoCfg(modalPagoInfo.metodo).color}`">
                  {{ metodoCfg(modalPagoInfo.metodo).icon }} {{ metodoCfg(modalPagoInfo.metodo).label }}
                </span>
              </div>
            </div>
...
.kpi-valor    { font-size: 1.35rem; font-weight: 800; color: #880e4f; margin: 0 0 0.1rem; }
...
.metodo-chips { display: flex; gap: 0.75rem; flex-wrap: wrap; }
...
.mc-monto { font-weight: 800; margin-left: 0.25rem; }
...
.tab-btn.activo .tab-count { background: rgba(255,255,255,0.25); color: white; }

/* Chips método */
.metodo-filtros { display: flex; gap: 0.4rem; flex-wrap: wrap; }
...
.metodo-chip-btn:hover { border-color: #f48fb1; color: #c2185b; }
.metodo-chip-btn.activo { border-color: #880e4f; background: #fce4ec; color: #880e4f; }
...
.id-badge.pago   { background: #fce4ec; color: #c2185b; }
.id-badge.pedido { background: #e8eaf6; color: #3949ab; }

.td-nombre { font-weight: 700; color: #880e4f; font-size: 0.875rem; margin: 0 0 0.12rem; }
.td-email  { color: #bbb; font-size: 0.75rem; display: block; }
.td-fecha  { font-size: 0.75rem; color: #aaa; white-space: nowrap; }
.td-null   { color: #ccc; font-style: italic; font-size: 0.8rem; }
...
.masked-card { display: block; font-size: 0.7rem; color: #aaa; margin-top: 0.15rem; font-family: monospace; }
...
.estado-pedido.pendiente   { background: #fff3e0; color: #e65100; border-color: #ffe0b2; }
.estado-pedido.entregado   { background: #e8f5e9; color: #2e7d32; border-color: #c8e6c9; }
.estado-pedido.cancelado   { background: #fce4ec; color: #c62828; border-color: #f8bbd0; }
...
.modal-info-item strong { font-size: 0.9rem; font-weight: 700; color: #880e4f; }
...
@media (max-width: 1100px) {
  .kpis-grid { grid-template-columns: 1fr repeat(4, 1fr); }
}
...
@media (max-width: 640px) {
  ...
  .filtros-wrap { flex-direction: column; align-items: stretch; }
