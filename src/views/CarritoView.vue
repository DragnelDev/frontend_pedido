        ...
        <div class="col-productos">
          ...
          >
            ...
            <div class="item-info">
              ...
              <span class="item-precio">Bs. {{ Number(item.producto.precio ?? 0).toFixed(2) }}</span>
            ...
            <button class="btn-eliminar" @click="eliminarProducto(item.producto.id)" aria-label="Eliminar">
  ...
  <Teleport to="body">
    <Transition name="modal-fade">
      <div
        ...
        @click.self="mostrarModalStock = false"
      >
        <div class="modal-box">
          ...
          <p class="modal-text">Algunos productos no tienen stock suficiente. Ajusta las cantidades:</p>
...
.vacio-icon { font-size: 3.5rem; margin-bottom: 1rem; opacity: 0.35; }
.carrito-vacio h4 { font-weight: 800; color: #1a0810; margin-bottom: 0.4rem; }
.carrito-vacio p { color: #a07080; margin-bottom: 1.5rem; font-size: 0.9rem; }
.btn-explorar {
  display: inline-flex;
  align-items: center;
  gap: 7px;
...
.btn-explorar:hover { opacity: 0.88; transform: translateY(-1px); color: #fff; }
...
.item-card:hover { box-shadow: 0 6px 20px rgba(232, 81, 122, 0.1); border-color: #f5a8c0; }
.item-card.item-error { border-color: #ffcdd2; background: #fff8f8; }
...
.item-info h6 { font-weight: 700; color: #1a0810; margin-bottom: 0.15rem; font-size: 0.92rem; }
.item-desc {
  font-size: 0.78rem;
  color: #c0a0b0;
...
.item-precio { font-weight: 700; color: #e8517a; font-size: 0.9rem; }
.stock-error-msg {
  display: flex;
  align-items: center;
  gap: 4px;
...
.btn-qty:hover { background: #fce8f0; border-color: #e8517a; }
.qty-val { font-weight: 700; color: #1a0810; min-width: 22px; text-align: center; font-size: 0.9rem; }
...
.btn-eliminar:hover { background: #ffcdd2; transform: scale(1.1); }
...
.btn-vaciar:hover { background: #fff0f0; }
...
.resumen-divider { height: 1px; background: #f5e8ee; margin: 0.9rem 0; }
.resumen-total {
  display: flex;
  justify-content: space-between;
  font-weight: 800;
...
.resumen-total span:last-child { color: #e8517a; }
...
.btn-pago:hover:not(:disabled) { opacity: 0.88; transform: translateY(-1px); }
.btn-pago:disabled { opacity: 0.6; cursor: not-allowed; }
...
.btn-seguir:hover { background: #fff4f8; border-color: #f5a8c0; }
...
.modal-title { font-size: 1.05rem; font-weight: 800; color: #1a0810; margin: 0 0 0.4rem; }
.modal-text { font-size: 0.85rem; color: #a07080; margin: 0 0 1rem; }
.stock-lista { list-style: none; padding: 0; margin: 0 0 1.25rem; text-align: left; }
.stock-lista li {
...
@media (max-width: 480px) {
  ...
  .carrito-page { padding: 1.5rem 1rem 3rem; }
}
</style>
